...
class BankAccount:
    def __init__(
        ...
        *,
        ...
        if (
            not self.is_overdraft_allowed
            and overdraft_amount > 0
            or overdraft_amount < 0
        ):
            raise ValueError(
                "is_overdraft_allowed is False, therefore setting overdraft amount is not allowed"
            )
    ...
    def ledger_wrap(fn):
        def inner(self, amount):
            _dict = {
                ...
                "transaction_reference": random.randrange(10**13, 10**14),
            }
